...
describe("given syslog messages", function()
  local messages = {
    ...
    },
    {
      ...
      input=[[<29>1 2016-02-21T04:32:57+00:00 web1 someservice - - [origin x-service="someservice"][meta sequenceId="14125553"] 127.0.0.1 - - 1456029177 "GET /v1/ok HTTP/1.1" 200 145 "-" "hacheck 0.9.0" 24306 127.0.0.1:40124 575]],
      valid=true,
    },
    {
    ...
    },
    {
      ...
      input=[[<78>1 2016-01-15T00:04:01+00:00 host1 CROND 10391 - [sdid x="⌘"] some_message]],
      valid=true,
    },
  }
  ...
  for _,message in ipairs(messages) do
    it(message.label, function()
      if message.valid then
        assert.is_equal(syslog.parse(message.input, message.input:len()), message.input:len())
      else
        assert.is_equal(syslog.parse(message.input, message.input:len()), -1)
      end
    end)
  end
end)
